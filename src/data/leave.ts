...
export const leaveRulesByCountry: Record<Country, LeaveType[]> = {
  BO: [
    ...
    { nombre: 'Licencia con goce', dias: 3, color: 'var(--color-info)' },
...
/** Saldos del usuario actual (Edgar, Bolivia). */
export const myBalances: LeaveBalance[] = [
  { tipo: 'Vacaciones', total: 15, usado: 5, color: 'var(--color-accent)' },
  { tipo: 'Licencia con goce', total: 3, usado: 0, color: 'var(--color-info)' },
...
function buildRequests(): LeaveRequest[] {
  ...
  const tipos = ['Vacaciones', 'Enfermedad', 'Licencia con goce', 'Matrimonio', 'Día administrativo']
